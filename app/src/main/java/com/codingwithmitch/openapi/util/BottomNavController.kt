...
import com.codingwithmitch.openapi.fragments.main.AccountNavHostFragment
...
class BottomNavController(
    ...
    fun setupBottomNavigationBackStack(previousBackStack: BackStack?) {
        navigationBackStack = previousBackStack?.let { it }?: BackStack.of(appStartDestinationId)
    }
    ...
    private fun createNavHost(menuItemId: Int): Fragment =
        when(menuItemId) {
            R.id.nav_account -> {AccountNavHostFragment.create(R.navigation.nav_account)}
            R.id.nav_create_blog -> {AccountNavHostFragment.create(R.navigation.nav_create_blog)}
            R.id.nav_blog -> {AccountNavHostFragment.create(R.navigation.nav_blog)}
            else -> {AccountNavHostFragment.create(R.navigation.nav_blog)}
        }
    ...
    fun onBackPressed() {
        ...
        when {
            fragmentManager.fragments.size > 2 -> {
